    ...
    if (clusterRef.current) {
      ...
      group.on("clusterclick", (e: any) => {
        ...
        const bounds = L.latLngBounds(
          childMarkers.map((m: any) => m.getLatLng()),
        );
...
function MapFitBounds({
  ...
  partners: Array<{ lat: number | null; lng: number | null }>;
}) {
  const map = useMap();
...
export default function PartnerDirectoryEN() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery<{ partners: Partner[]; total: number }>({
    queryKey: [
      "/api/partners",
      {
        limit: 1000,
    ...
    queryFn: async () => {
      try {
        const response = await fetch(
          "/api/partners?limit=1000&status=Approved&is_active=true&show_on_map=true&transform=true",
          {
            cache: "no-store",
  ...
  // Scroll to top when page loads (important for footer navigation)
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: "instant" });
  }, []);

  // Force white color for header subtitle using DOM manipulation (CSS classes sometimes fail to apply)
  useEffect(() => {
    const timeout = setTimeout(() => {
      const subtitle = document.querySelector(
        '[data-testid="header-subtitle-en"]',
      );
      if (subtitle) {
        (subtitle as HTMLElement).style.setProperty(
          "color",
          "#FFFFFF",
          "important",
        );
  ...
  // Partners with coordinates for map (ensure all have unique slugs)
  const mappablePartners = ensureUniquePartnerSlugs(
  ...
  const serviceCounts = allServices.map((service) => ({
    id: service.id,
    name: service.label,
    count: visiblePartners.filter((p) => p.services.includes(service.id))
      .length,
  }));
    ...
    <div className="min-h-screen bg-[#F2EBDC]">
      ...
      <div className="bg-gradient-to-r from-[#2A4759] to-[#011526] text-white py-6">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl font-bold mb-2">
            Digitization Services Directory
          </h1>
          <p className="text-xl text-white" data-testid="header-subtitle-en">
      ...
      <div className="container mx-auto px-4 py-4">
        ...
        <div className="mb-4 flex flex-col lg:flex-row items-start lg:items-center gap-3 lg:w-1/2">
          {/* Search Bar */}
          <div
            className="relative w-full lg:w-auto lg:flex-shrink-0"
              ...
              {serviceCounts.map(({ id, name, count }) => {
                ...
                return (
                  <Badge
                    key={id}
                    variant={
                      selectedServices.includes(id) ? "default" : "outline"
                    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="h-[350px] lg:h-[588px] rounded-lg overflow-hidden shadow-lg relative">
              ...
              <div
                className="absolute top-4 right-4 z-[1000] bg-[#D67C4A] text-white px-4 py-2.5 rounded-lg shadow-xl flex items-center gap-2 font-semibold text-base lg:text-lg"
                data-testid="visible-partners-counter"
              >
                <MapPin className="h-5 w-5 lg:h-6 lg:w-6" />
                <span>
                  {visiblePartners.length} professional
                  {visiblePartners.length !== 1 ? "s" : ""} found
                </span>
              </div>
              <MapContainer
                center={mapCenter}
          ...
          <div
            id="partner-list-container"
            className="flex flex-col gap-3 max-h-[600px] lg:h-[588px] overflow-y-auto snap-y snap-mandatory partner-list-scrollbar p-0 pr-6"
            style={{
              background:
                "linear-gradient(to right, transparent calc(100% - 24px), #F2EBDC calc(100% - 24px))",
          ...
          >
            {isLoading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="mt-2">
                  Zoom or pan the map to see more partners.
                </p>
              </div>
            ) : (
              <>
                      ...
                      <CardContent className="p-0 h-full flex flex-col">
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 text-[#F2EBDC]">
                                <MapPin
                                  className="h-4 w-4"
                                  data-testid={`partner-location-icon-${partner.slug}`}
                                  ref={(el) => {
                                    if (el)
                                      el.style.setProperty(
                                        "stroke",
                                ...
                                <span
                                  className="text-sm"
                                  data-testid={`partner-location-${partner.slug}`}
                                  ref={(el) => {
                                    if (el)
                                      el.style.setProperty(
                                        "color",
                        ...
                        >
                          ...
                          {partner.public_description && (
                            <div>
                              <p
                                className={`text-sm text-gray-700 leading-snug ${!isExpanded ? "line-clamp-2" : ""}`}
                              >
                                {partner.public_description}
                              </p>
                              {!isExpanded &&
                                partner.public_description.length > 100 && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setExpandedCard(partner.slug);
                          ...
                          <div
                            className={`space-y-1.5 ${isExpanded ? "border-t pt-2" : ""}`}
                          >
                            {isExpanded && (
                              <h4 className="font-semibold text-gray-900 text-xs mb-1">
                                Services
                            ...
                            {["Photo", "Film", "Video"].map((service) => {
                              ...
                              const formats =
                                service === "Photo"
                                  ? partner.formats.photo
                                  : service === "Film"
                                    ? partner.formats.film
                              ...
                              return (
                                ...
                                >
                                  <Badge
                                    className={`${hasService ? "bg-[#D67C4A] text-white hover:bg-[#c5703e]" : "bg-gray-300 text-gray-500 opacity-50"} px-2 py-0.5 shrink-0 w-[90px] text-xs flex items-center justify-center gap-1`}
                                  >
                                    <Icon className="h-3 w-3" />
                                    {service}
                                  </Badge>
                                  <div
                                    className={`flex flex-wrap gap-1 ${isExpanded ? "mt-1" : "flex-1"}`}
                                  >
                                    {hasService ? (
                                      <>
                          ...
                          <div
                            className={`border-t flex flex-wrap items-center gap-x-4 gap-y-1 ${isExpanded ? "pt-2" : "pt-2"}`}
                          >
                            {partner.website && (
                              <a
                                href={
      ...
      <Dialog
        open={!!modalPartner}
        onOpenChange={(open) => !open && setModalPartner(null)}
      >
            ...
            <>
              <DialogHeader>
                ...
                <div className="flex items-center gap-2 text-gray-600 mt-2">
                  ...
                  <span>
                    {modalPartner.city}, {modalPartner.country}
                  </span>
                </div>
              </DialogHeader>
                  ...
                  <div className="space-y-3">
                    {modalPartner.services
                      ...
                      .map((service) => {
                        const formats =
                          service === "Photo"
                            ? modalPartner.formats.photo
                            : service === "Film"
                              ? modalPartner.formats.film
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {modalPartner.website && (
                      <a
                        href={
                          modalPartner.website.startsWith("http")
                            ? modalPartner.website
                            : `https://${modalPartner.website}`
                        }
                        target="_blank"
                        rel="noopener noreferrer"
