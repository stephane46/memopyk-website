...
export default function BlogIndexPage() {
  ...
  // Scroll to top when blog index page loads
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  // Fix: Force white text on blog header using DOM manipulation (CSS classes not applying)
  useEffect(() => {
    const timeout = setTimeout(() => {
      const subtitle = document.querySelector('[data-testid="text-blog-subtitle"]');
      const description = document.querySelector('[data-testid="text-blog-description"]');
      
      if (subtitle) {
        (subtitle as HTMLElement).style.setProperty('color', '#FFFFFF', 'important');
      }
      if (description) {
        (description as HTMLElement).style.setProperty('color', '#FFFFFF', 'important');
      }
    }, 50);
        ...
        <header className="relative bg-[#2A4759] py-4 md:py-6 overflow-hidden">
          <div className="container mx-auto px-4 relative z-10">
            ...
            <div className="mt-4 max-w-3xl">
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-['Playfair_Display'] font-bold leading-tight text-white" data-testid="text-blog-title">
                {t.title}
              </h1>
              <p className="text-lg md:text-xl mt-2 font-medium text-white" data-testid="text-blog-subtitle">
                {t.subtitle}
              </p>
              <p className="text-sm md:text-base mt-1.5 leading-relaxed text-white/90" data-testid="text-blog-description">
                {t.description}
              </p>
            </div>
          </div>
