...
export default function VideoOverlay({
  ...
  const getOrCreateSessionId = useCallback(() => {
    ...
    // CRITICAL FIX: Don't overwrite page session ID - video uses its own tracking
    // localStorage.setItem("memopyk-current-session-id", clientSessionId); // REMOVED
    ...
    async (forceImmediate = false) => {
      ...
      try {
        ...
        const progressPct = Math.max(
          0,
          Math.min(100, Math.round((video.currentTime / videoDuration) * 100)),
        );
  ...
  const handleCanPlay = useCallback(() => {
    ...
    if (video && showThumbnail) {
      ...
      const remainingTime = Math.max(
        0,
        MINIMUM_THUMBNAIL_DISPLAY_TIME - timeElapsed,
      );
  ...
  const handleCloseWithAnalytics = useCallback(() => {
    ...
    if (!isNaN(actualDuration) && actualDuration > 0 && actualCurrentTime > 0) {
      ...
      const completionRate = Math.round(
        (actualCurrentTime / actualDuration) * 100,
      );
  ...
  return (
    <div
      ref={overlayRef}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md transition-all duration-300 ease-out video-overlay-dialog"
    ...
    >
      ...
      <button
        onClick={onClose}
        className="md:hidden fixed top-4 right-4 z-[60] bg-black/80 backdrop-blur-sm text-white p-3 rounded-full shadow-lg hover:bg-black/90 transition-colors"
        aria-label="Close video"
        data-testid="button-close-video-mobile"
      >
        <X size={24} />
      </button>
      ...
      >
        ...
        {showThumbnail && thumbnailUrl && (
          ...
          >
            ...
            {/* Centered overlays - Using 100% isolated vov-* CSS namespace */}
            <div className="vov-overlay-wrapper">
              <div className="vov-content-wrapper vov-fade-in">
                {sourceText && (
                  <div className="vov-source-container">
                    <div className="vov-source-pill">
                      <div className="vov-source-text">
                        <ImageIcon className="vov-source-icon" />
                        <span className="vov-text-span" style={{ color: '#FFFFFF' }}>{sourceText}</span>
                      </div>
                      <div className="vov-client-badge">
                        {language === "fr-FR"
                          ? "fournies par client"
                          : "provided by client"}
                      </div>
                ...
                {stableProps.title && (
                  <div className="vov-title-container">
                    <div className="vov-title-wrapper">
                      <h3 className="vov-title-text" style={{ color: '#FFFFFF !important' }}>
                        {stableProps.title}
                      </h3>
                      {durationText && (
                        <div className="vov-duration-text-inline">
                          <Clock className="vov-duration-icon" />
                          <span className="vov-text-span" style={{ color: '#FFFFFF' }}>{durationText}</span>
                        </div>
                      )}
                    </div>
                  </div>
