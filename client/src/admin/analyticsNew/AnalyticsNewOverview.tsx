...
export const AnalyticsNewOverview: React.FC<AnalyticsNewOverviewProps> = ({ 
  ...
  // Get current filter state including data source
  const { datePreset, customDateStart, customDateEnd, sinceDate, sinceDateEnabled, getDateRange, dataSource, setDataSource } = useAnalyticsNewFilters();
  ...
  return (
    <div className={`analytics-new-container space-y-6 ${className}`}>
      {/* Header with Active Users Badge and Data Source Toggle */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          {/* Data Source Toggle */}
          <div className="flex items-center space-x-3 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
            <Label htmlFor="data-source-toggle" className="text-sm font-medium text-gray-700 cursor-pointer">
              Data Source:
            </Label>
            <div className="flex items-center space-x-2">
              <span className={`text-xs font-medium ${dataSource === 'ga4' ? 'text-blue-600' : 'text-gray-400'}`}>
                GA4
              </span>
              <Switch
                id="data-source-toggle"
                checked={dataSource === 'memopyk'}
                onCheckedChange={(checked) => setDataSource(checked ? 'memopyk' : 'ga4')}
                data-testid="toggle-data-source"
              />
              <span className={`text-xs font-medium ${dataSource === 'memopyk' ? 'text-orange-600' : 'text-gray-400'}`}>
                MEMOPYK
              </span>
            </div>
            <Badge 
